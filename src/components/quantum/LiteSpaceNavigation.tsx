...
const NavigationCursor = memo(({ containerRef }: { containerRef: React.RefObject<HTMLDivElement> }) => {
  ...
  useEffect(() => {
    ...
    if (containerRef.current) {
      ...
      containerRef.current.addEventListener('mouseout', () => setIsHovering(false))
    }
    ...
    return () => {
      if (containerRef.current) {
        ...
        containerRef.current.removeEventListener('mouseout', () => setIsHovering(false))
      }
    }
  }, [containerRef])
  ...
  return (
    ...
    >
      <div className="relative -translate-x-1/2 -translate-y-1/2">
        <div className={`h-6 w-6 rounded-full border-2 ${isHovering ? 'border-blue-400' : 'border-gray-400'}`}>
...
export const LiteSpaceNavigation: React.FC<LiteSpaceNavigationProps> = memo(({ isOpen, onClose }) => {
  ...
  }, [isOpen, onClose, handleNavigation])
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            <motion.button
              onClick={onClose}
              className="absolute right-6 top-6 z-50 rounded-full bg-white/10 p-3 backdrop-blur-sm transition-all hover:bg-white/20"
              ...
              {filteredItems.map((item, index) => (
                ...
                >
                  ...
                  {selectedPlanet === item.id && (
                    <motion.div
                      className="absolute left-1/2 top-full mt-2 -translate-x-1/2 whitespace-nowrap rounded-lg bg-white/10 px-3 py-2 backdrop-blur-sm"
            ...
            >
              <p className="text-sm text-gray-400">
                <Command className="mb-1 inline h-4 w-4" /> Use numbers 1-7 for quick navigation
              </p>
