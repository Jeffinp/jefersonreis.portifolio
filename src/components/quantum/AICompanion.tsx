...
export const AICompanion: React.FC<AICompanionProps> = ({ enabled = true }) => {
  ...
  const [animationState, setAnimationState] =
    useState<AnimationState>('floating')
  ...
  const [easterEggs, setEasterEggs] = useState<EasterEgg[]>([
    {
      ...
      description: 'You clicked on me!',
    },
    {
      id: 'ten-clicks',
      unlocked: false,
      title: 'Persistent Friend',
      description: "10 clicks! We're best friends now!",
    },
    {
      id: 'konami',
      unlocked: false,
    ...
    },
    {
      ...
      unlocked: false,
      title: 'Scroll Master',
      description: "You've explored everything!",
    },
  ])
  ...
  const unlockEasterEgg = (eggId: string) => {
    setEasterEggs((prev) =>
      prev.map((egg) => (egg.id === eggId ? { ...egg, unlocked: true } : egg)),
    )
    setCurrentMessage({
      id: Date.now(),
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const scrollDirection =
        scrollY > scrollPositionRef.current ? 'down' : 'up'
  ...
  }, [enabled, easterEggs, showMessage, unlockEasterEgg])
  ...
  const MiniGame = () => (
    <motion.div
      ...
      className="bg-cosmic-black/90 fixed inset-0 z-[60] flex items-center justify-center backdrop-blur-lg"
    >
      <div className="holographic max-w-md rounded-2xl p-8">
        ...
        <p className="mb-6 text-gray-400">
          Coming soon! For now, enjoy this achievement!
        </p>
        ...
        <button
          ...
          className="bg-stellar-blue hover:bg-stellar-blue/80 w-full rounded-lg px-4 py-2 font-bold text-white transition-colors"
        >
          Close
        </button>
      </div>
  ...
  return (
    <>
      ...
      >
        ...
        >
          {/* Glow Effect */}
          <div className="bg-stellar-blue/20 absolute -inset-4 rounded-full blur-xl" />

          {/* Main Body */}
          <div className="border-stellar-blue bg-cosmic-black/90 relative h-16 w-16 rounded-2xl border-2 backdrop-blur-sm">
            {/* Face */}
            <div className="absolute inset-2 flex items-center justify-center">
              {/* Eyes */}
              <div className="flex gap-2">
                <motion.div
                  className="bg-stellar-blue h-2 w-2 rounded-full"
                  animate={{
              ...
              <div className="absolute bottom-2">
                {mood === 'happy' && (
                  <div className="bg-stellar-blue h-1 w-4 rounded-full" />
                )}
                {mood === 'excited' && (
                  <Heart className="text-nova-pink h-3 w-3" />
                )}
                {mood === 'curious' && (
                ...
                {mood === 'thinking' && (
                  <div className="flex gap-0.5">
                    <div className="bg-stellar-blue h-1 w-1 rounded-full opacity-50" />
                    <div className="bg-stellar-blue h-1 w-1 rounded-full opacity-70" />
                    <div className="bg-stellar-blue h-1 w-1 rounded-full" />
                  </div>
                )}
              </div>
            </div>
            ...
            >
              <div className="bg-stellar-blue h-3 w-0.5" />
              <motion.div
                className="bg-stellar-blue absolute -top-1 -left-1 h-2 w-2 rounded-full"
        ...
        <AnimatePresence>
          {currentMessage && (
            ...
            >
              ...
              <div className="border-stellar-blue/30 bg-cosmic-black/80 absolute -bottom-2 left-8 h-4 w-4 rotate-45 border-r border-b" />
            </motion.div>
          )}
        </AnimatePresence>
            ...
            >
              <div className="holographic rounded-xl p-4">
                ...
                <div className="space-y-2">
                  <button className="bg-stellar-blue/10 hover:bg-stellar-blue/20 flex w-full items-center gap-2 rounded-lg p-2 text-left transition-colors">
                    <Rocket className="text-stellar-blue h-4 w-4" />
                    <span className="text-sm text-white">Quick Tour</span>
                    <ChevronRight className="ml-auto h-4 w-4 text-gray-400" />
                  </button>
                  <button className="bg-stellar-blue/10 hover:bg-stellar-blue/20 flex w-full items-center gap-2 rounded-lg p-2 text-left transition-colors">
                    <Code className="text-stellar-blue h-4 w-4" />
                    <span className="text-sm text-white">View Source Code</span>
                    <ChevronRight className="ml-auto h-4 w-4 text-gray-400" />
                  </button>
                  <button
                    onClick={() => setShowMiniGame(true)}
                    className="bg-stellar-blue/10 hover:bg-stellar-blue/20 flex w-full items-center gap-2 rounded-lg p-2 text-left transition-colors"
                  >
                    <Trophy className="text-stellar-blue h-4 w-4" />
                    <span className="text-sm text-white">Achievements</span>
                    <span className="ml-auto text-xs text-gray-400">
                      {easterEggs.filter((e) => e.unlocked).length}/
                      {easterEggs.length}
                    </span>
                  </button>
                </div>
      ...
      <motion.button
        className="bg-stellar-blue/20 hover:bg-stellar-blue/30 fixed bottom-8 left-8 z-[34] rounded-full p-3 backdrop-blur-sm transition-colors"
        onClick={() => setIsExpanded(!isExpanded)}
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 0.9 }}
      >
        <Bot className="text-stellar-blue h-5 w-5" />
      </motion.button>

      {/* Mini-game Modal */}
      <AnimatePresence>{showMiniGame && <MiniGame />}</AnimatePresence>
    </>
  )
}
