...
const LiteProjectCard = memo(({ project, onClick }: { project: Project; onClick: () => void }) => {
  const { t } = useTranslation(['projects/web-projects', 'projects/mobile-projects', 'projects/design-projects', 'projects/3d-projects'])
  
  // Use titleKey and descriptionKey if available, otherwise fallback to old format
  const projectTitle = project.titleKey ? t(project.titleKey, { ns: ['projects/web-projects', 'projects/mobile-projects', 'projects/design-projects', 'projects/3d-projects'] }) : (project.title || t(`projects.${project.id}.title`, project.id))
  const projectDescription = project.descriptionKey ? t(project.descriptionKey, { ns: ['projects/web-projects', 'projects/mobile-projects', 'projects/design-projects', 'projects/3d-projects'] }) : (project.description || t(`projects.${project.id}.description`, ''))
  ...
  return (
    ...
    >
      ...
      <div className="relative h-48 w-full overflow-hidden bg-gray-800 cursor-pointer" onClick={onClick}>
        {imageSrc ? (
          <Image
            src={imageSrc}
            alt={projectTitle}
      ...
      <div className="p-4">
        ...
        <h3 className="mb-2 text-lg font-bold text-white cursor-pointer hover:text-blue-400 transition-colors" onClick={onClick}>
          {projectTitle}
        </h3>
...
const LiteProjectModal = memo(({ 
  ...
  const { t } = useTranslation(['projects/web-projects', 'projects/mobile-projects', 'projects/design-projects', 'projects/3d-projects', 'main'])
  ...
  const projectTitle = project.titleKey ? t(project.titleKey, { ns: ['projects/web-projects', 'projects/mobile-projects', 'projects/design-projects', 'projects/3d-projects'] }) : (project.title || t(`projects.${project.id}.title`, project.id))
  const projectDescription = project.descriptionKey ? t(project.descriptionKey, { ns: ['projects/web-projects', 'projects/mobile-projects', 'projects/design-projects', 'projects/3d-projects'] }) : (project.description || t(`projects.${project.id}.description`, ''))
  const projectDetailedDescription = project.fullDescriptionKey ? t(project.fullDescriptionKey, { ns: ['projects/web-projects', 'projects/mobile-projects', 'projects/design-projects', 'projects/3d-projects'] }) : (project.fullDescription || project.detailedDescription || projectDescription)
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            <button
              onClick={onClose}
              className="absolute right-4 top-4 z-10 rounded-full bg-gray-800/50 p-2 text-gray-400 backdrop-blur-sm transition-colors hover:bg-gray-700/50 hover:text-white"
            >
              <X className="h-5 w-5" />
            </button>
            ...
            <div className="mb-4">
              <div className="mb-2 flex items-center gap-3">
                <span className={`rounded-full px-3 py-1 text-xs ${
                  ...
                  {project.type === 'contracted' ? 'Projeto Comercial' : 'Projeto Pessoal'}
                </span>
                <span className="text-xs text-gray-500 uppercase tracking-wider">
                  {project.category}
                </span>
              </div>
              <h2 className="mb-2 text-2xl font-bold text-white">{projectTitle}</h2>
            ...
            <div className="mb-6">
              <h3 className="mb-2 text-lg font-semibold text-white">Sobre o Projeto</h3>
              <p className="text-gray-300 leading-relaxed">{projectDetailedDescription}</p>
            </div>
            ...
            <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
              {project.completionDate && (
                <div className="rounded-lg bg-gray-800/50 p-3">
                  ...
                  <p className="text-sm font-medium text-white">{project.completionDate}</p>
                </div>
              )}
              {project.teamSize && (
                <div className="rounded-lg bg-gray-800/50 p-3">
                  ...
                  <p className="text-sm font-medium text-white">{project.teamSize} {project.teamSize > 1 ? 'pessoas' : 'pessoa'}</p>
                </div>
              )}
              {project.duration && (
                <div className="rounded-lg bg-gray-800/50 p-3">
              ...
              {project.technologies && (
                <div className="rounded-lg bg-gray-800/50 p-3">
                  ...
                  <p className="text-sm font-medium text-white">{project.technologies.length} techs</p>
                </div>
              )}
            </div>
            ...
            {project.technologies && (
              <div className="mb-6">
                <h3 className="mb-3 text-lg font-semibold text-white">Tecnologias Utilizadas</h3>
            ...
            {project.challenges && project.challenges.length > 0 && (
              <div className="mb-6">
                <h3 className="mb-3 text-lg font-semibold text-white">Desafios Enfrentados</h3>
            ...
            {project.results && project.results.length > 0 && (
              <div className="mb-6">
                <h3 className="mb-3 text-lg font-semibold text-white">Resultados Alcançados</h3>
                <ul className="space-y-2">
...
const LiteQuantumProjects: React.FC = memo(() => {
  ...
  const categories = useMemo(() => [
    ...
    { id: 'design', label: t('projects.categories.design'), icon: <Palette /> },
  ], [t])
