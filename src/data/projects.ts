...
export const projects: Project[] = [
...

  {
    ...
    longDescription:
      'Como sócio e CTO, desenhei toda a arquitetura e lidero a engenharia da plataforma que conecta pacientes a profissionais de saúde domiciliar — do zero, com foco em escalabilidade, segurança e compliance regulatório.\n\nBackend (NestJS 11 + PostgreSQL 15/PostGIS): 13 módulos especializados — auth, users, professionals, addresses, search, appointments, ratings, notifications, payments, metrics, health, audit e redis. API híbrida REST + GraphQL Code-First (Apollo). JWT com refresh tokens em Redis blacklist. Segurança em camadas: Helmet, CORS, Throttler (10/s, 100/min, 1000/h) + limite de 5 tentativas de login. AuditInterceptor registra todas as mutations.\n\nIntegrações: Stripe Payment Intent + webhook para pagamentos com confirmação idempotente. Twilio para SMS/OTP. DiDiT para KYC (reconhecimento facial). Busca geoespacial via PostGIS com view materializada v_enderecos_completos.\n\nMobile (Expo Router + React Native): App iOS/Android com Zustand stores segmentados (auth, appointment, payment, notification, location), tokens em expo-secure-store, Stripe Payment Sheet e tema anti-flash com SystemUI nativo.\n\nAdmin Panel (React 19 + Vite + Refine): Dashboard para aprovação de profissionais e auditoria.\n\nDevOps: VPS Hostinger KVM2 (8GB RAM). Docker Compose com resource limits, read-only filesystem, seccomp e no-new-privileges. Nginx como reverse proxy com SSL Let\'s Encrypt. Backup automático diário do PostgreSQL com retenção de 7 dias. Prometheus + Terminus para observabilidade.',
    category: 'mobile',
    status: 'in-progress',
    featured: true,
    order: 3,
    ...
    startDate: '2025-09',
    challenges: [
      'Segurança multicamada: Helmet, CORS, Throttler com 3 janelas (10/s, 100/min, 1000/h), AuditInterceptor em todas as mutations e PII masking em logs',
      'API híbrida REST + GraphQL: REST para webhooks e fluxos lineares (Stripe, uploads), GraphQL Code-First com Dataloader para telas densas sem over-fetching',
      'Pagamentos com idempotência: Stripe Payment Intent criado no backend, confirmação exclusivamente via webhook assinado — nenhum estado financeiro vem do cliente',
      'Busca geoespacial: view materializada v_enderecos_completos com geography(Point,4326) para encontrar profissionais próximos em milissegundos via PostGIS',
    ],
    results: [
      'Backend com 13 módulos de domínio com separação estrita — cada módulo com controller, service, repository, guards e testes próprios',
      'App mobile anti-flash com ThemeProvider + SystemUI nativo, tokens em expo-secure-store e Zustand stores com persistência seletiva por domínio',
      'Infraestrutura hardened na VPS: Docker Compose com resource limits, read-only filesystem, seccomp, no-new-privileges e backup automático diário (retenção 7 dias)',
      'Observabilidade completa: Prometheus em /metrics (Basic Auth), Terminus em /health monitorando PostgreSQL, Redis, heap e disco',
    ],
  },
